...
export class AnalyticsService {
  ...
  async getAdminOverview() {
    ...
    const encadreurs = await this.userRepo
      ...
      .getCount();
    const verified = await this.userRepo.count({
    ...
    const subjectDraft = await this.subjectRepo.count({
      where: { status: SubjectStatus.DRAFT },
    });
    ...
    const subjectRejected = await this.subjectRepo.count({
      where: { status: SubjectStatus.REJECTED },
    });
    const subjectAi = await this.subjectRepo.count({
    ...
    const candidaturePending = await this.candidatureRepo.count({
      where: { status: CandidatureStatus.PENDING },
    });
    const candidatureAccepted = await this.candidatureRepo.count({
      where: { status: CandidatureStatus.ACCEPTED },
    });
    const candidatureRejected = await this.candidatureRepo.count({
      where: { status: CandidatureStatus.REJECTED },
    });
    ...
    const stageActive = await this.stageRepo.count({
      where: { status: StageStatus.ACTIVE },
    });
    const stageCompleted = await this.stageRepo.count({
      where: { status: StageStatus.COMPLETED },
    });
    const stageCancelled = await this.stageRepo.count({
      where: { status: StageStatus.CANCELLED },
    });
    const stagesWithoutPro = await this.stageRepo.count({
      where: { encadrantProId: IsNull() },
    });
    const stagesWithoutAcad = await this.stageRepo.count({
      where: { encadrantAcadId: IsNull() },
    });

    // ── Jalons ──
    const totalJalons = await this.jalonRepo.count();
    const jalonPending = await this.jalonRepo.count({
      where: { status: JalonStatus.PENDING },
    });
    const jalonSubmitted = await this.jalonRepo.count({
      where: { status: JalonStatus.SUBMITTED },
    });
    const jalonValidated = await this.jalonRepo.count({
    ...
    const studentsWithCv = await this.profileRepo.count({
      where: { isAiProcessed: true },
    });
    let subjectsIndexed = 0;
    try {
      const rows = await this.dataSource.query(
        'SELECT COUNT(*) as count FROM subject_embeddings',
      );
  ...
  async getEncadreurOverview(encadreurId: string) {
    ...
    if (myStageIds.length > 0) {
      const jalonAgg = await this.jalonRepo
        ...
        .select('COUNT(*)', 'total')
        .addSelect(
          `SUM(CASE WHEN j.status = '${JalonStatus.SUBMITTED}' THEN 1 ELSE 0 END)`,
          'submitted',
        )
        .addSelect(
          `SUM(CASE WHEN j.status = '${JalonStatus.VALIDATED}' THEN 1 ELSE 0 END)`,
          'validated',
        )
        .addSelect(
  ...
  async getEncadreurMyStudents(encadreurId: string) {
    ...
    const results = await Promise.all(
      stages.map(async (stage) => {
        ...
        const validatedJalons = jalons.filter(
          (j) => j.status === JalonStatus.VALIDATED,
        ).length;
        ...
        const upcoming = jalons
          .filter(
            (j) =>
              new Date(j.dueDate) >= now && j.status !== JalonStatus.VALIDATED,
          )
          .sort(
            (a, b) =>
        ...
        const completionPercentage =
          totalJalons > 0
            ? Math.round((validatedJalons / totalJalons) * 100)
            : 0;
        ...
        const lastActivity: Date | null =
          lastLivrable?.livrable?.submittedAt ?? null;
        ...
        return {
          ...
          jalons: {
            total: totalJalons,
            validated: validatedJalons,
            overdue: overdueJalons,
            nextDeadline,
          },
  ...
  async getStudentOverview(studentId: string) {
    ...
    if (stage) {
      const daysRemaining = stage.endDate
        ? Math.round(
            (new Date(stage.endDate).getTime() - now.getTime()) /
              (1000 * 60 * 60 * 24),
          )
        : null;
      stageData = {
    ...
    if (stage) {
      ...
      const validated = jalons.filter(
        (j) => j.status === JalonStatus.VALIDATED,
      ).length;
      const pending = jalons.filter(
        (j) => j.status === JalonStatus.PENDING,
      ).length;
