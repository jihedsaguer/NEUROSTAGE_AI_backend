...
export class AssignmentsService {
  ...
  async assignEncadreurToStudent(
    encadreurId: string,
    studentId: string,
  ): Promise<AssignmentResponseDto> {
    const encadreur = await this.userRepository.findOne({
      where: { id: encadreurId },
      relations: ['roles'],
    });
    if (!encadreur) {
    ...
    const isEncadreur = encadreur.roles.some((role) =>
      role.name.includes('encadreur'),
    );
    if (!isEncadreur)
      throw new NotFoundException(
        `User with email ${encadreur.email} is not an encadreur`,
      );
    ...
    if (existingAssignment) {
      throw new ConflictException(
        'This student is already assigned to this encadreur',
      );
    }
